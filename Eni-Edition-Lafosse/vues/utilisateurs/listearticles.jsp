...
<%@ include file="../outils/entete.jspf" %>
	...
	<div id="breadcrumbs">
		<c:if test="${recherche == null || recherche == ''}" >	
			Cat&eacute;gorie &gt; 
			<a href="<b:config attribut="urlApplication"/>gestionArticles?action=lister&idCategorie=<c:out value="${categorie.id}"/>">
				<c:out value="${categorie.nom}"/>
			</a>
		</c:if>
		<c:if test="${recherche != null && recherche != ''}" >	
			R&eacute;sultat de votre recherche pour "<c:out value="${recherche}" />"
		</c:if>
	</div>
		...
		<tr>
			<td>
				<p:pagination servlet="gestionArticles" pageActuel="${pageActuel}" totalElement="${totalElement}" maxParPage="${maxParPage}" champTri="${champTri}" typeTri="${typeTri}">
					...
					<p:paramServlet valeur="${categorie.id}" nom="idCategorie"/>
					...
					<table class="article">
						...
						<tr>
							<td class="description" colspan="2">
								<b:resumer longueur="25" texte="${article.description}"/>
							</td>
						</tr>
						<tr>
